...
| **T6** | `compras-erp-preload` (hoje cron no Trigger.dev) → pg_cron + Edge Function do Supabase | — | ✅ **feito no codigo** (2026-07-15, Codex): Edge Function `supabase/functions/compras-erp-preload`, cron SQL `migrations/024_compras_erp_preload_edge_cron.sql`, task antiga removida do Trigger; falta aplicar/deployar na VPS |
