...
| **T6** | `compras-erp-preload` (hoje cron no Trigger.dev) → pg_cron + Edge Function do Supabase | — | ✅ **feito e publicado na VPS** (2026-07-15, Codex): Edge Function em `/opt/supabase/volumes/functions/compras-erp-preload`, cron `pg_cron` ativo como job `4`, task antiga removida do Trigger; endpoint protegido por `COMPRAS_ERP_PRELOAD_SECRET` |
...
| **T8** | Limpar `trigger.config.ts`/env não usadas (agora sobram só `erp-foto-sync` e `expedicaoSync` no Trigger) | T6 | parcial: deploy Trigger `20260715.20` subiu com 2 tasks detectadas |
