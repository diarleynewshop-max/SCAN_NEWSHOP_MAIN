...
export async function anexarPdfNaTaskClickUp(
  taskId: string,
  empresa: string,
  pdf: Pick<PedidoFornecedorPdf, "dataUrl" | "filename">
): Promise<void> {
  const response = await fetch(`/api/clickup-compras-proxy?action=anexar-pdf-pedido&empresa=${empresa}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ taskId, pdfBase64: pdf.dataUrl, filename: pdf.filename }),
  });

  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data?.error || `Erro ${response.status} ao anexar PDF no ClickUp`);
  }
}

export async function buscarPdfAnexadoNaTask(
  taskId: string,
  empresa: string
): Promise<{ dataUrl: string; filename: string } | null> {
  const response = await fetch(`/api/clickup-compras-proxy?action=buscar-pdf-pedido&empresa=${empresa}&taskId=${taskId}`);
  if (response.status === 404) return null;
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data?.error || `Erro ${response.status} ao buscar PDF anexado`);
  }
  const data = await response.json();
  if (!data.pdfBase64) return null;
  return { dataUrl: data.pdfBase64, filename: data.filename ?? `pedido_${taskId}.pdf` };
}

export function baixarPdfDataUrl(dataUrl: string, filename: string): void {
  const link = document.createElement("a");
  link.href = dataUrl;
